  ...
  <a-modal
    ...
    >
      <a-form-item label="客户" >
        <j-dict-select-tag v-model="name" placeholder="请选择客户名称"  dictCode="zm_client_main,username,username"/>
      </a-form-item>
      <a-form-item label="运单">
        <a-upload
          name="file"
          :multiple="true"
          accept=".xls,.xlsx"
    ...
    <template slot="footer">
      ...
      <a-button
        type="primary"
        @click="handleImport"
        :disabled="fileList.length === 0||this.name===''||this.name===null"
        :loading="uploading">
        {{ uploading ? '上传中...' : '开始上传' }}
      </a-button>
    </template>
