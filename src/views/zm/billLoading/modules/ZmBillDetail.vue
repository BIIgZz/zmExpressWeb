  ...
  <page-layout :title="title">
    <a-card :bordered="false" :title="title">
      ...
      <detail-list title="基础信息">
        <detail-list-item term="类型">{{billDtail}}</detail-list-item>
        <detail-list-item term="提单号">{{billDtail.billnum}}</detail-list-item>
        <detail-list-item term="船名">{{billDtail.billnum}}</detail-list-item>
        <detail-list-item term="航次">{{billDtail.billnum}}</detail-list-item>
        ...
        <detail-list-item term="工作号">{{billDtail.billnum}}</detail-list-item>
...
import { downFile, handleDetailss } from '../../../../api/manage'
...
export default {
  ...
  methods:{
    ...
    handleExportExcel(fileName){
      ...
      console.log("导出")
      downFile("/zmexpress/zmBillloading/exportCustomsInformation","1480053901199032322").then((data)=>{
        if (!data) {
        ...
        if (typeof window.navigator.msSaveBlob !== 'undefined') {
          ...
          link.href = url
          link.setAttribute('download', this.dataFba+'.xls')
